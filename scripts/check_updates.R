...
write_etag_cache <- function(etag, last_modified) {
  ...
  write_json(
    list(etag = etag, last_modified = last_modified, checked_at = format(Sys.time(), "%Y-%m-%dT%H:%M:%SZ")),
...
check_cricsheet_updates <- function() {
  ...
  resp <- tryCatch({
    request(CRICSHEET_URL) |> req_method("HEAD") |> req_timeout(30) |> req_perform()
  }, error = function(e) {
    cli_alert_danger("Failed to check Cricsheet: {e$message}")
    NULL
  })

  if (is.null(resp)) return(list(needs_update = TRUE, reason = "HEAD request failed"))
  ...
  result <- if (is.null(cached_etag)) {
    ...
    list(needs_update = TRUE, reason = "ETag changed")
  } else {
    cli_alert_success("No changes detected (ETag unchanged)")
    list(needs_update = FALSE, reason = "ETag unchanged")
  }

  if (!is.null(current_etag)) write_etag_cache(current_etag, current_modified)

  c(result, list(etag = current_etag, last_modified = current_modified, content_length = content_length))
}
